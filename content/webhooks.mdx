...
| Interviewer evaluation | `interview.interviewer_evaluation` | Interviewer performance analysis (1-to-1 HUMAN interviews only) |
...
### `interview.interviewer_evaluation`

Fired after interviewer performance analysis completes. **1-to-1 HUMAN interviews only** — never sent for AI interviews or team meetings. Useful for tracking interviewer quality across your organization. Visible only to OWNER/ADMIN in the Boooply dashboard; never shown to the interviewer themselves.

```json
{
  "event": "interview.interviewer_evaluation",
  "data": {
    "meetingCode": "Boooply-H-1234567890",
    ...
    "interviewerEvaluation": {
      "overallRating": 4,
      "overallSummary": "Strong interview with clear structure and good rapport...",
      "confidenceLevel": "HIGH",
      "confidenceReasoning": "42-min transcript, multiple probing questions observed",
      "questionQuality": {
        "rating": 4,
        "openEndedCount": 6,
        "closedEndedCount": 2,
        ...
        "bestQuestion": "Walk me through a tradeoff decision on your last launch",
        "worstQuestion": "Do you like working in teams? (closed yes/no)",
        "evidence": "6 open-ended questions, probing follow-ups throughout"
      },
      "biasAssessment": {
        "rating": 5,
        "identifiedBiases": [],
        "evidence": "No bias patterns detected across the transcript"
      },
      "timeManagement": {
        "rating": 4,
        "isInterviewerDominant": false,
        "rushedTopics": [],
        "missedTopics": []
      },
      "candidateExperience": {
        "rating": 5,
        "welcomingAndRapport": 5,
        "clearExpectationSetting": 4,
        "activeListening": 5,
        "appropriateFollowUps": 5,
        "closingAndNextSteps": 4
      },
      "professionalConduct": {
        "rating": 5,
...
| Field | Type | Description |
|-------|------|-------------|
| `interviewerEvaluation.overallRating` | `number \| null` | 1–5 scale. Null when `confidenceLevel` is `LOW`. |
| `interviewerEvaluation.confidenceLevel` | `string` | `HIGH` \| `MEDIUM` \| `LOW`. Use this to filter sparse-transcript evaluations. |
| `interviewerEvaluation.biasAssessment.identifiedBiases` | `array` | Empty array when no bias is detected (not `null`, not a placeholder). |
| `interviewerEvaluation.professionalConduct.legalRiskQuestions` | `array` | Any questions touching age, family, religion, health, origin. If non-empty, `professionalConduct.rating` is capped at 2. |
| `analysisMeta.durationMs` | `number` | Wall-clock time (ms) the analysis run took. |
...
```json
{
  ...
  "data": {
    ...
    "videoApiUrl": "https://api.boooply.com/api/integration/interviews/Boooply-AI-1234567890/recording",
    "timestamp": "2026-03-21T18:40:00.000Z"
  }
}
```
